 ...
 *
 * HOW TO FIX AN OFFENDER (Next.js)
 * --------------------------------------
 * Next auto-prefixes ONLY `next/link` href and `next/image` src (plus its own
 * bundle/asset URLs), because next.config.ts sets basePath/assetPrefix from
 * NEXT_PUBLIC_BASE_PATH. Nothing else is touched.
 *
 * Use NEXT_PUBLIC_BASE_PATH in your own code:
 *
 *     const base = process.env.NEXT_PUBLIC_BASE_PATH ?? "";
 *     const res = await fetch(`${base}/api/items`);
 *     <img src={`${base}/logo.svg`} />
 *
 * fleet.conf sets NEXT_PUBLIC_BASE_PATH at BOTH build and start, so it is
 * available to server and client code alike.
 *
 * EXEMPTING A LINE
 * ----------------
 ...
 *
 *     src="/next.svg" // next/image prefixes basePath - base-path-ok
 *
